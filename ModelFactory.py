...
class ModelFactory:
    ...
    def _load_w_array(self):
        ...
        for i in range(len(temp) - 1):
            wp = np.random.normal(self.random_mu, self.random_sigma,
                                  (temp[i], temp[i + 1])).astype(dtype = theano.config.floatX)  # .astype(dtype = theano.config.floatX)
            bp = np.random.normal(self.random_mu, self.random_sigma,
                                  (1, temp[i + 1])).astype(dtype = theano.config.floatX)  # .astype(dtype = theano.config.floatX)
            # wp = np.random.randn( temp[i], temp[i + 1])
            # bp = np.random.randn(1, temp[i + 1])
            bp = np.tile(bp, (self.batch_num, 1))
    ...
    def _create_model(self):
        ...
        for i in range(len(self.W_array)):
            result2 = ModelFactory._layer_propagate(result2, self.W_array[i], self.B_array[i][0])

        result = tensor.exp(result)
        self.y_evaluated = result/tensor.sum(result)
        result2 = tensor.exp(result2)
        self.y_evaluated_output = result2/tensor.sum(result2)
    ...
    def _define_update_function(self):
        ...
        self.cost = -1*((tensor.log(self.y_evaluated)*self.y_input).sum())
    ...
    @staticmethod
    def _act_function(x):
        return tensor.clip(x,0,10000000)
    ...
    def load_parm(self, w_input, b_input,vW_input,vB_input):
        temp = [self.input_dim] + self.layer_neuron_num_list + [self.output_dim]
        for i in range(len(temp) - 1):
